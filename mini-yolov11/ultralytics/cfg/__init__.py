...
# The mini package keeps only YOLOv8 object detection. Solutions, tracking,
# segmentation, classification, pose, OBB, semantic, and export-specific task
# frontends are intentionally not exposed.
SOLUTION_MAP = {"help": None}
...
def entrypoint(debug: str = "") -> None:
    ...
    model = YOLO(model, task=task or "detect")
    # Task Update
    if task != model.task:
        if task:
            LOGGER.warning(
